//归并排序  //归 （分治 （递归）） 并（合并）
// 1，申请一个数组空间，使其大小为两个已经排序序列之和，该空间用来存放合并后的序列；
...
public class MergeSort {
    ...
    static  void merge(int[] list1,int[] list2,int[] temp){ // 三个数组
        int current1=0;
        int current2=0;
        int current3=0;

        while (current1< list1.length && current2< list2.length){
            if(list1[current1]< list2[current2])  //把 比较两个数组
                temp[current3++]=list1[current1++];
            else
                temp[current3++]=list2[current2++];
        }

        while (current1<list1.length)  //把未比较的数组元素 直接加入目标数组
            temp[current3++]=list1[current1++];
